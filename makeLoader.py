...
for isig in range(0,len(data.list_mc_sig)):
   cdir=os.getcwd()
   if data.FileInlxplusEOS=="Yes":
       fileN="eos ls "+data.fileLocation_sig+data.list_mc_sig[isig]+ " > Sig.dat"
   if data.FileInlpcEOS=="Yes":
   ...
   create ="./MainSig.sh %s%s %s %s"%(data.store_mc_sig,FNameList[0],data.treeName_sig,"ClassReadSig")
   os.system(create)
...
for ibkg in range(0,len(data.list_mc_bkg)):
   cdir=os.getcwd()
   if data.FileInlxplusEOS=="Yes":
       fileN="eos ls "+data.fileLocation_bkg+data.list_mc_bkg[ibkg]+ " > Bkg.dat"
   if data.FileInlpcEOS=="Yes":
   ...
   with open("Bkg.dat") as f:
      FNameList = f.readlines()
   FNameList = [x.strip() for x in FNameList] 
   create ="./MainBkg.sh %s%s %s %s"%(data.store_mc_bkg,FNameList[0],data.treeName_bkg,"ClassReadBkg")
   os.system(create)
...
for idata in range(0,len(data.list_data)):
   cdir=os.getcwd()
   if data.FileInlxplusEOS=="Yes":
       fileN="eos ls "+data.fileLocation_data+data.list_data[idata]+ " > Data.dat"
   if data.FileInlpcEOS=="Yes":
   ...
   with open("Data.dat") as f:
      FNameList = f.readlines()
   FNameList = [x.strip() for x in FNameList] 
   create ="./MainData.sh %s%s %s %s"%(data.store_data,FNameList[0],data.treeName_data,"ClassReadData")
   os.system(create)
